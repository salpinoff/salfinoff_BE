...
| createdAt | LocalDateTime | 회원 생성일 +
...
| encryptedMonsterId | String | 암호화된 몬스터 고유 id | O
| monsterName | String | 몬스터 이름 | O
| ownerName | String | 몬스터 주인 이름 | O
| interactionCount | int | 인터렉션 수 | O
| currentInteractionCount | int | 현재 인터렉션 수 | O
| interactionCountPerEncouragement | int | 한 번의 응원 당 부여되는 인터렉션 수 +
(서브 유저가 응원을 보내기 위해 인터렉션 해야 하는 횟수) | O
| ratingRange | RANGE_1: 1~25 사이의 rating +
RANGE_2: 26~50 사이의 rating +
RANGE_3: 51~75 사이의 rating +
RANGE_4: 76~100 사이의 rating |
rating 범위 | O
| rating | int | 스트레스 수치 | O
| content | String | 친구들에게 공유할 메시지 | O
| emotion | String | 감정 +
종류: DEPRESSION, ANGER | O
| createdAt | LocalDateTime | 몬스터 생성일 +
패턴: yyyy-MM-dd'T'HH:mm:ss | O
...
operation::monster-details-read-encrypted[snippets='http-request']
Path Params
|===
| key | type | description | required
| encryptedMonsterId | String | 암호화된 몬스터 고유 id | O
...
operation::monster-details-read-encrypted[snippets='http-response']
|===
| key | type | description | required
| monsterId | Long | 몬스터 고유 id (pk) | O
| encryptedMonsterId | String | 암호화된 몬스터 고유 id | O
...
=== 암호화된 몬스터 id 복호화
operation::monster-decrypt-id[snippets='http-request']
Path Params
|===
| key | type | description | required
| encryptedMonsterId | String | 암호화된 몬스터 고유 id | O
...
operation::monster-decrypt-id[snippets='http-response']
Response body
|===
| key | type | description | required
| monsterId | Long | 몬스터 고유 id (pk) | O
...
| size | int | 한 페이지에 출력되는 컨텐츠 개수 | O
| page | int | 페이지 | O
| totalElements | long | 총 컨텐츠 개수 | O
| checkedMessageCount | long | 확인된 메시지 개수 | O
| uncheckedMessageCount | long | 미확인 메시지 개수 | O
|===
...
* 토큰 재발급 api 반환 값 추가
- 1.0.4 (2024-06-06)
* 몬스터 상세 조회 반환 값에 몬스터 주인 이름 추가
* 몬스터 감정 ANGER 추가
* 몬스터 상세 조회 반환 값에 서브 유저 인터렉션 카운트 값 추가
- 1.0.5 (2024-06-09)
* 몬스터 상세 조회 반환 값에 rating 범위, 생성일 추가
- 1.0.6 (2024-06-25)
* 몬스터 메시지 목록 조회 반환 값에 확인 메시지 개수, 미확인 메시지 개수 추가
- 1.0.7 (2024-08-15)
* 몬스터 암호화 키 추가
* 몬스터 암호화 키 복호화 api 추가
* 몬스터 암호화 키로 몬스터 조회 api 추가